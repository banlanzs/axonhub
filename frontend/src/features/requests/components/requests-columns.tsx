...
// ============================================================
// Memoized cell components (reuse the channels-columns pattern)
// ============================================================
...
const IdCell = memo(function IdCell({ row, onBodyClick }: IdCellProps) {
  return (
    <button
      onClick={() => onBodyClick?.(row.original.id, row.index)}
      className='text-primary cursor-pointer font-mono text-xs hover:underline'
    >
      #{extractNumberID(row.original.id)}
    </button>
  );
});
IdCell.displayName = 'IdCell';

const ModelIDCell = memo(function ModelIDCell({ row }: { row: Row<Request> }) {
...
ModelIDCell.displayName = 'ModelIDCell';

const ApiFormatCell = memo(function ApiFormatCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
ApiFormatCell.displayName = 'ApiFormatCell';

const PassThroughCell = memo(function PassThroughCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
PassThroughCell.displayName = 'PassThroughCell';
...
ReasoningEffortCell.displayName = 'ReasoningEffortCell';

const StreamCell = memo(function StreamCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
const SourceCell = memo(function SourceCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
const ClientIPCell = memo(function ClientIPCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
ClientIPCell.displayName = 'ClientIPCell';

const StatusCell = memo(function StatusCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
...
const TokensCell = memo(function TokensCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
  const request = row.original;
...
TokensCell.displayName = 'TokensCell';

const ReadCacheCell = memo(function ReadCacheCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
  const request = row.original;
  const usageLog = request.usageLogs?.edges?.[0]?.node;
...
ReadCacheCell.displayName = 'ReadCacheCell';

const WriteCacheCell = memo(function WriteCacheCell({ row }: { row: Row<Request> }) {
  const { t } = useTranslation();
  const request = row.original;
  const usageLog = request.usageLogs?.edges?.[0]?.node;
...
WriteCacheCell.displayName = 'WriteCacheCell';

const CostCell = memo(function CostCell({ row }: { row: Row<Request> }) {
...
const LatencyCell = memo(function LatencyCell({ row, displayMode }: LatencyCellProps) {
  const { t } = useTranslation();
  const request = row.original;
  const latencyParts = [];
...
LatencyCell.displayName = 'LatencyCell';

const CreatedAtCell = memo(function CreatedAtCell({ row }: { row: Row<Request> }) {
...
const ApiKeyCell = memo(function ApiKeyCell({ row }: ApiKeyCellProps) {
  const { t } = useTranslation();
...
export function useRequestsColumns(options?: UseRequestsColumnsOptions): ColumnDef<Request>[] {
    ...
    return [
      {
        ...
        cell: ({ row }) => <IdCell row={row} onBodyClick={options?.onBodyClick} />,
        enableSorting: true,
        enableHiding: false,
      },
      {
      ...
      },
      {
        ...
        enableSorting: false,
        enableHiding: true,
        cell: ({ row }) => <ApiFormatCell row={row} />,
      },
      {
        id: 'passThrough',
        ...
        enableSorting: false,
        enableHiding: true,
        cell: ({ row }) => <PassThroughCell row={row} />,
      },
      {
        ...
        enableSorting: false,
        enableHiding: true,
        cell: ({ row }) => <ReasoningEffortCell row={row} />,
      },
      {
        id: 'stream',
        accessorKey: 'stream',
      ...
      },
      {
        ...
        cell: ({ row }) => <ReadCacheCell row={row} />,
        enableSorting: true,
        enableHiding: true,
        sortingFn: (rowA, rowB) => {
      ...
      },
      {
        ...
        cell: ({ row }) => <WriteCacheCell row={row} />,
        enableSorting: true,
        enableHiding: true,
        sortingFn: (rowA, rowB) => {
      ...
      },
      {
        ...
        cell: ({ row }) => <LatencyCell row={row} displayMode={displayMode} />,
        enableSorting: true,
        enableHiding: true,
        sortingFn: (rowA, rowB) => {
  ...
  }, [t, locale, permissions.canViewChannels, permissions.canViewApiKeys, displayMode, handleToggleDisplayMode, navigateWithSearch, options?.onBodyClick, options?.onViewDetail]);
